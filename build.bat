...
set DIST=%ROOT%dist\RandomMovie_1.0.0.0
...
echo Packaging zip...
powershell -NoProfile -Command "Compress-Archive -Path '%DIST%\*' -DestinationPath '%ROOT%dist\RandomMovie-1.0.0.0.zip' -Force"

echo.
echo Done. Zip: %ROOT%dist\RandomMovie-1.0.0.0.zip
echo Install: copy dist\RandomMovie_1.0.0.0 fajlat a jellyfin plugin mappaba, majd inditsd ujra a szervert.
endlocal